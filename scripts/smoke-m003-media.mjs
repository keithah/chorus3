...
export const DEFAULT_M003_MEDIA_SMOKE_PORT = 8080;
export const DEFAULT_M003_MEDIA_SMOKE_PATH = '/jsonrpc';
export const DEFAULT_M003_MEDIA_SMOKE_TIMEOUT_MS = 5000;
export const MEDIA_SMOKE_LIMITS = { start: 0, end: 5 };
export const MEDIA_SMOKE_METHODS = [
  'JSONRPC.Ping',
...
const MEDIA_PROBES = [
  {
    ...
    params: { limits: MEDIA_SMOKE_LIMITS, properties: ['thumbnail'] },
    resultKey: 'artists'
  },
  {
    method: 'AudioLibrary.GetAlbums',
    params: { limits: MEDIA_SMOKE_LIMITS, properties: ['artist', 'year', 'thumbnail'] },
    resultKey: 'albums'
  },
  {
    method: 'AudioLibrary.GetSongs',
    params: { limits: MEDIA_SMOKE_LIMITS, properties: ['artist', 'album', 'duration'] },
    resultKey: 'songs'
  },
  {
  ...
  },
  {
    ...
    params: { directory: 'special://musicplaylists', media: 'music', limits: MEDIA_SMOKE_LIMITS },
    resultKey: 'files',
    successLine: 'Smart playlist root: listed read-only metadata only.'
  }
];
...
function normalizePath(path) {
  if (!hasValue(path)) {
    return DEFAULT_M003_MEDIA_SMOKE_PATH;
  }
...
function createEndpointDescription(url, timeoutMs, hasCredentials) {
  return {
    ...
    host: url.hostname,
    port: Number(url.port || (url.protocol === 'https:' ? 443 : DEFAULT_M003_MEDIA_SMOKE_PORT)),
    path: `${url.pathname || DEFAULT_M003_MEDIA_SMOKE_PATH}${url.search}`,
    timeoutMs,
    hasCredentials
  };
}
...
function parseUrlEndpoint(rawUrl, timeoutMs, hasCredentials) {
  ...
  if (!url.pathname || url.pathname === '/') {
    url.pathname = DEFAULT_M003_MEDIA_SMOKE_PATH;
  }
...
function buildSplitEndpoint(sourceEnv, timeoutMs, hasCredentials) {
  ...
  const port =
    parsePositiveInteger(sourceEnv.KODI_PORT, 'KODI_PORT') ?? DEFAULT_M003_MEDIA_SMOKE_PORT;
  if (port > 65535) {
...
export function parseM003MediaSmokeEnv(sourceEnv = env) {
  ...
  try {
    ...
    const timeoutMs =
      ...
      DEFAULT_M003_MEDIA_SMOKE_TIMEOUT_MS;
    ...
    return {
      ok: false,
      code: 'invalid-env',
      lines: [`M003 media smoke configuration error: ${safeErrorMessage(error)}`]
    };
  }
}
  ...
  try {
    ...
    if (!response.ok) {
      return createFailure(
        ...
        `M003 media smoke failed for ${config.endpoint}: HTTP ${response.status} ${response.statusText || 'Error'} while calling ${method}.`
      );
    }
    ...
    try {
      ...
      return createFailure(
        'malformed-response',
        `M003 media smoke failed for ${config.endpoint}: Kodi returned an invalid JSON response while calling ${method}.`
      );
    }
    ...
    if (!envelope || typeof envelope !== 'object' || envelope.jsonrpc !== '2.0') {
      return createFailure(
        'malformed-response',
        `M003 media smoke failed for ${config.endpoint}: Kodi returned a malformed JSON-RPC envelope while calling ${method}.`
      );
    }
    ...
    if ('error' in envelope) {
      ...
      return createFailure(
        'json-rpc-error',
        `M003 media smoke failed for ${config.endpoint}: JSON-RPC ${code} while calling ${method}.`
      );
    }
    ...
    if (!('result' in envelope)) {
      return createFailure(
        'malformed-response',
        `M003 media smoke failed for ${config.endpoint}: Kodi JSON-RPC envelope did not include a result while calling ${method}.`
      );
    }
    ...
    if (
      ...
      return createFailure(
        'timeout',
        `M003 media smoke failed for ${config.endpoint}: ${method} timed out after ${config.timeoutMs}ms.`
      );
    }

    return createFailure(
      'network',
      `M003 media smoke failed for ${config.endpoint}: Could not reach Kodi while calling ${method}.`
    );
  } finally {
...
function validateProbeResult(method, result, resultKey) {
  ...
  if (count === undefined) {
    return createFailure(
      'malformed-response',
      `M003 media smoke failed: ${method} returned an unexpected result shape.`
    );
  }
...
export async function runM003MediaSmoke(config, { fetchImpl = fetch } = {}) {
  ...
  const lines = [
    `M003 media smoke succeeded for ${config.endpoint}.`,
...
export async function runM003MediaSmokeCli(sourceEnv = env) {
  const parsed = parseM003MediaSmokeEnv(sourceEnv);

  if (!parsed.ok || parsed.skipped) {
    return parsed;
  }

  return runM003MediaSmoke(parsed.config);
}
...
if (invokedPath === modulePath) {
  const result = await runM003MediaSmokeCli();
