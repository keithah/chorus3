...
describe('VideoMoviesPanel', () => {
  ...
  it('renders movie cards with count watched resume versions and finite detail links', () => {
    ...
    expect(
      document
        .querySelector<HTMLImageElement>('[aria-label="Alien artwork availability"] img')
        ?.getAttribute('src')
    ).toBe('/image/image%3A%2F%2Fposter-alien%2F');
  ...
  it('omits non-finite detail links and unsafe path-like artwork or labels', () => {
    ...
    expect(document.querySelector('.fallback-initials')?.textContent).toBe('SM');
